...
def test_stationary(timeseries, plot_results=False, data_frequency=None, force_transformation_technique=None, custom_transformation_size=None):
    '''
    使用ADF(Augmented Dickey-Fuller)测试来检查时序是否平稳

    本函数会执行不同的转换（log transformation, difference, seasonal difference等），然后会应用ADF测试。
    测试结束之后，函数会基于最小显著性差异测试返回最好的转换数据（以及其它一些数据）
    ...
    Args:
        timeseries (Pandas Series): 用于统计测试和转换的时序序列
        plot_results (bool): 是否为最优时序转换绘图
        data_frequency (str, optional): 数据手机的时间频率 (daily, monthly, yearly, etc.)
        force_transformation_technique (str, optional): 强制转换使用的转换技巧名，此参数由在sidebar中所选择的参数传入
